BP<-read.table("ByP_P_mirnas.txt",header=T)
names(BP) = gsub(pattern = ".tailed_reads", replacement = "", x = names(BP))
...
for( i in c("R-10b-5p","R-10a-5p","R-143-3p","R-191-5p","R-451-5p","R-486-5p","R-1388-5p","R-320a-2-3","R-126-5","R-127-3")) {
  BP%>%
    filter(str_detect(BP$miR.name,i))-> TCV
